...
const sections = [
  ...
  { key: 'hostings', title: 'الاستضافات', endpoint: 'hostings', fields: [['company_name','اسم الشركة','text'],['domain','الدومين','text'],['current_space_mb','المساحة الحالية MB','number'],['allowed_space_mb','المساحة المسموح بها','number'],['domain_renewal_cost_sar','تكلفة تجديد النطاق','number'],['hosting_cost_sar','تكلفة الاستضافة','number'],['record_date','التاريخ','date'],['tax_sar','الضريبة','number'],['hosting_renewal_date','تاريخ تجديد الهوست','date'],['domain_renewal_date','تاريخ تجديد الدومين','date'],['notes','ملاحظات','text']] },
];
...
export default function App() {
  ...
  const totals = (payload && (payload.totals || payload.summary)) || {};
  ...
  async function load() {
    ...
    catch (error) { Alert.alert('خطأ', (error && error.message) || 'تعذر تحميل البيانات'); }
    finally { setLoading(false); }
  }

  function emptyForm() {
    const next = {};
    section.fields.forEach((field) => { next[field[0]] = field[2] === 'date' ? today() : ''; });
    return next;
  }
  ...
  function edit(row) {
    ...
    section.fields.forEach((field) => { next[field[0]] = row[field[0]] == null ? '' : String(row[field[0]]); });
  ...
  async function save() {
    try {
      const body = { ...form };
      section.fields.forEach((field) => { if (field[2] === 'number' && !body[field[0]]) body[field[0]] = '0'; });
      if (editing && editing.id) await call(section.endpoint + '/' + editing.id, { method: 'PUT', body: JSON.stringify(body) });
      else await call(section.endpoint, { method: 'POST', body: JSON.stringify(body) });
      setModal(false); load();
    } catch (error) { Alert.alert('تعذر الحفظ', (error && error.message) || 'راجع البيانات'); }
  }
...
function Card({ label, value }) { return <View style={styles.card}><Text style={styles.key}>{label}</Text><Text style={styles.big}>{String(value == null ? 0 : value)}</Text></View>; }
...
echo "COM mobile converted to plain App.js with clean app.json"
